...
simulated function Tick(float Delta)
{
	...
	if (TorsoAimer != None && bUseAimOffset)
	{
		...
		if (CRZMonsterController(Controller).Target != None)
			TV = CRZMonsterController(Controller).Target.Location;
		...
		else
		{
			...
			{
				CRZMonsterController(Controller).Target.GetBoundingCylinder(CR2, CH2);
...
{
	if ( Role == ROLE_Authority && bIsLunging && Pawn(Other)!=None && Controller!=None && Other == CRZMonsterController(Controller).Target )
	{
		bIsLunging = false;
...
simulated function MeleeDamage()
{
	...
	if (CRZMonsterController(Controller).Target != None && ROLE == ROLE_Authority)
	{
		if (VSize(CRZMonsterController(Controller).Target.Location - Location) <= AttackDistance)
			CRZMonsterController(Controller).Target.TakeDamage(PunchDamage, Controller, CRZMonsterController(Controller).Target.Location, V, None);
	}
}
...
reliable server function DoShot(name BoneName)
{
	...
	FinalRotation = rotator(Normal(CRZMonsterController(Controller).Target.Location - FinalLoc));
...
event TakeDamage(int Damage, Controller InstigatedBy, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo HitInfo, optional Actor DamageCauser)
{
	...
	{
		CRZMonsterController(Controller).LockOnTo(InstigatedBy.Pawn);
...
DefaultProperties
{
	...
	ControllerClass=class'ToxikkInvasion.CRZMonsterController'

	// Monsters can't jump... yet
    // bJumpCapable=false
    // bCanJump=false
